...
var g_extraOffsetToCaterForAd = 0;


// PATH TO THE TRANSPARENT 1*1 PX IMAGE (required by NS 4 as spacer)
var spacer = '/lib/transparentpixel.gif';
...
function my_DragFunc()
{
    ...
    var s_name = dd.obj.name;
    for (i = 1; i <= 12; i++)
    {
        var elm_guess = document.getElementById("guess" + i);
        if (elm_guess.value == s_name)
        {
            elm_guess.value = "";
            break;
        }
...
function my_DropFunc()
{
    ...
    var x = 0;
    var y = 0;
    var x_off = 25 + elmAll.offsetLeft;
    var y_off = 90 + g_extraOffsetToCaterForAd;
    var tile_x_max = 4;
    var tile_y_max = 3;
    var tile_x = dd.obj.x + tilesize / 2;
    var tile_y = dd.obj.y + tilesize / 2;

    tile_count = 0;
    for (x = 0; x < tile_x_max; x++)
    {
        var x_min = (x_off) + (x * tilesize);
        var x_max = (x_off) + ( (x + 1) * tilesize );

        if (tile_x >= x_min && tile_x <= x_max)
        {
            for (y = 0; y < tile_y_max; y++)
            {
                var y_min = (y_off) + (y * tilesize);
                var y_max = (y_off) + ( (y + 1) * tilesize );

                if (tile_y >= y_min && tile_y <= y_max)
                {
                    dd.obj.moveTo( (x_off) + x * tilesize, (y_off) + y * tilesize );

                    tile_count = (x + 1) + ( y * tile_x_max );
                    var elm_guess = document.getElementById("guess" + tile_count);

                    var old_tile = elm_guess.value;
                    if (old_tile != "")
                    {
                        var my_item = dd.elements[old_tile];
                        my_item.moveTo(520, 40);
                    }

                    elm_guess.value = dd.obj.name;
